...
export default function GameController() {
  ...
  const clientSendUserAction = (action: ActionType, rotation?: Rotation) => {
  ...
  const clientCreateNewGame = async () => {
    try {
      const response = await serverCreateNewGame();
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex w-full justify-between">
        <Tooltip text={'Create a new game'}>
          <GameButton onClick={() => clientCreateNewGame()}>
            <AddIcon />
          </GameButton>
        </Tooltip>

        <Tooltip text={'Restart game'}>
          <GameButton onClick={() => clientSendUserAction('reset')}>
            <RestartIcon />
          </GameButton>
        </Tooltip>
      </div>
              ...
              <GameButton
                ...
                onClick={() =>
                  parse.data.action === 'rotate'
                    ? clientSendUserAction(
                        parse.data.action,
                        parse.data.rotation
                      )
                    : clientSendUserAction(parse.data.action)
                }
              >
                {parse.data.action === 'rotate' ? (
